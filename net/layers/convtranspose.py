...
class ConvTranspose(Layer):
    ...
    def on_input_shape(self):
        ...
        self.output_shape = (*tuple(np.add(self.input_shape[:2], self.kernel_size) - 1), self.filters)
    ...
    def forward(self, input):
        ...
        return np.reshape([
            np.sum(
                signal.correlate2d(self.input[:,:,i], self.kernels[k,:,:,i], mode='full')
                for i in range(self.channels)
            )
            for k in range(self.filters)
        ], self.output_shape)/self.input.size
    ...
    def backward(self, output_gradient):
        ...
        # print(kernels_gradient)


        input_gradient = np.reshape(np.sum([
            [
                signal.convolve2d(output_gradient[:,:,k], self.kernels[k,:,:,i], mode='valid')
                for i in range(self.channels)
            ]
            for k in range(self.filters)
